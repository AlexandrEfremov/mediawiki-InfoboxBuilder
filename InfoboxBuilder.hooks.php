...
final class InfoboxBuilderHooks {
	...
	public static function registerScribuntoLibraries( $engine, array &$extraLibraries ) {
		$extraLibraries['mw.InfoboxBuilderHF']   = 'LuaLibrary';
		$extraLibraries['mw.InfoboxBuilderView'] = 'LuaLibrary';
		$extraLibraries['mw.InfoboxBuilder']     = 'LuaLibrary';
	...
	public static function setupParserHook( Parser $parser ) {
		$parser->setFunctionHook( 'infoboxbuilder', 'InfoboxBuilderHooks::parserFunctionHook', SFH_OBJECT_ARGS );
		return true;
	}
	...
	public static function parserFunctionHook( Parser $parser, $frame, $args ) {
		...
		try {
			/**
			 * Add the registered LESS with the default theme
			 */
			$parser->getOutput()->addModuleStyles( 'ext.InfoboxBuilder.styles' );
			...
			$result = UtfNormal\Validator::cleanUp( strval( $result ) );
	...
	public static function addInfoboxBuilderStyles( EditPageLayoutController $controller ) {
		$controller->getContext()->getOutput()->addModuleStyles( 'ext.InfoboxBuilder.styles' );
		return true;
	}
}
